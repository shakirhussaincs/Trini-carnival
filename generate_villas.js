...
const mappedItems = oldProps.map(p => {
  const priceTtd = p.pricePerNight * 6.8;
  return {
    id: p.id,
    name: p.name,
    tagline: p.tagline,
    ...
    capacity: p.capacity,
    bedroomDetail: \`\${p.bedrooms} Bedrooms\`,
    priceTTD: \`$\${Math.floor(priceTtd)} - $\${Math.floor(priceTtd + 500)} TTD\`,
    priceUSD: \`$\${p.pricePerNight} - $\${p.pricePerNight + 50} USD\`,
    badge: p.badge,
    ...
    bedroomLayout: JSON.stringify(Array.from({length: p.bedrooms}).map((_, i) => ({
        title: \`Bedroom \${i+1}\`,
        type: i === 0 ? 'Ensuite' : 'Standard',
        bed: '1 Queen Bed'
    })))
  };
});

let dump = JSON.stringify(mappedItems, null, 2);
fs.writeFileSync('/tmp/dump.json', dump);
